<article class="myworld flex justify-between h-fit p-3 mx-1 my-4 bg-black outline outline-1 outline-white/20 rounded" data-id="{{ $friend->id }}">
    <div class="flex">
        <img src={{$friend->getImage()}} alt="{{$friend->persistentUser->member->username}} image" class="mobile:h-14 tablet:h-16 desktop:h-20 h-12 aspect-square">
        <div class="flex flex-col self-center ml-3 w-11/12">
            <h2 class="break-words"><a href="/members/{{ $friend->id }}">{{ $friend->presistentUser->members->username }}</a></h2>
