...
<article class="homepage" data-id="{{ $member->id }}">
    @if(count($tasks) > 0)
    <h2 class="decoration-green underline underline-offset-4 decoration-2" id="my-assigned-tasks-home"><a @if (Auth::user()->persistentUser->type_ != 'Blocked')href="{{route('show-tasks-list')}}"@endif>MY ASSIGNED TASKS </a></h2>
    <div class="panel">
    ...
    @endif
    @if(count($projects) > 0)
    <h2 class="decoration-green underline underline-offset-4 decoration-2" id="my-current-projects-home"><a @if (Auth::user()->persistentUser->type_ != 'Blocked')href="{{route('show-projects-list')}}"@endif>MY CURRENT PROJECTS</a> </h2>
    <div class="panel">
    ...
    @if(count($worlds) > 0)
        <div class="flex">
            <h2 class="decoration-green underline underline-offset-4 decoration-2" id="my-current-worlds-home"><a @if (Auth::user()->persistentUser->type_ != 'Blocked')href="{{route('show-worlds-list')}}"@endif> MY CURRENT WORLDS</a></h2>
    ...
    @else
        ...
        <div class="panel">
            <div class="worldsContainer">
                <h2> You do not belong to any worlds yet! </h2>
                @if (Auth::user()->persistentUser->type_ != 'Blocked')<h2><a class="underline text-green" href="/create-world">Create a new World</a></h2>@endif
            </div>
        </div>
    @endif
</article>
