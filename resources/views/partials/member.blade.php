<article class="member" data-id="{{ $member->id }}">
    ...
    <header class="h-fit flex justify-start">
        ...
        @if ($member->persistentUser->type_ == 'Member' || $member->persistentUser->type_ == 'Blocked') <h4 class="self-center" style="text-shadow: 0 0 10px green"><a href="/members/{{ $member->persistentUser->user->username }}">{{ $member->persistentUser->user->username }}</a></h4>
